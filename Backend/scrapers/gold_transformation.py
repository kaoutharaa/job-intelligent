...
def _write_table(table_name: str, df: pd.DataFrame, on_conflict: str = None) -> int:
    ...
    timeout=PIPELINE_CONFIG["TIMEOUT_SECONDS"],
        ...
        if resp.status_code == 409:
            log.warning(f"[GOLD] Duplicate conflict on {table_name}, chunk ignored")
...
def transform_silver_to_gold() -> Dict[str, int]:
    ...
    results[gold_tables["snapshots"]] = _write_table(
        gold_tables["snapshots"],
        daily_snapshot,
        on_conflict="snapshot_date,source,title_standardized",
    )
