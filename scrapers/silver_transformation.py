...
# SILVER LAYER: Cleaned, validated, and enriched data from Bronze (raw jobs)

# Functions:
#   - transform_bronze_to_silver(): Main transformation pipeline
#   - Cleans, normalizes, and validates all fields
#   - Extracts skills, standardizes job titles
#   - Stores results in Supabase silver_jobs table
# """
...
#     generate_job_id, write_to_supabase
# )
...
#     """
#     Read BRONZE (raw jobs) → Transform → Write SILVER (cleaned jobs)
#     """
#     log.info("[SILVER] Starting Bronze → Silver transformation...")
    
#     # 1. Extract Bronze (raw data)
#     log.info("[SILVER] Extracting from Bronze (jobs table)...")
#     all_rows = []
...
#             log.error(f"[SILVER] Extract error: {resp.status_code}")
#             break
...
#     log.info(f"[SILVER] Bronze extracted: {len(df_bronze)} rows")
    
#     # 2. Transform data
#     log.info("[SILVER] Transforming data...")
...
#         "date_posted", "scraped_at", "job_url", "salary_min", "salary_max", "salary_currency",
#         "contract_type", "source", "keywords", "job_category", "seniority_level",
...
#     log.info(f"[SILVER] Transformed {len(df_silver)} rows")
    
#     # 3. Write to Silver table (upsert)
#     _write_silver_jobs(df_silver)
    
#     return df_silver

# def _write_silver_jobs(df: pd.DataFrame) -> None:
#     """Write/upsert records to silver_jobs table"""
#     if df.empty:
#         log.warning("[SILVER] No records to write")
#         return
    
#     log.info(f"[SILVER] Writing {len(df)} records to {PIPELINE_CONFIG['SILVER_TABLE']} table...")
    
#     # Convert DataTypes to JSON-serializable format
#     df_json = df.copy()
#     df_json["keywords"] = df_json["keywords"].apply(
#         lambda x: x if isinstance(x, list) else []
#     )
#     df_json["validation_errors"] = df_json["validation_errors"].apply(
#         lambda x: x if isinstance(x, list) else []
#     )
...
#     # Upsert: POST with on_conflict resolution
...
#             log.error(f"[SILVER] Write error: {resp.status_code} - {resp.text[:300]}")
#         else:
